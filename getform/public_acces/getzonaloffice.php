<?php
ob_start();
session_start();
include_once('../../login_system/_inc/config.php');
$qmini = null;
$user_email = null;
$uleveldis = null;
$province = null;
$ministry = null;
$district_offce = null;
$divi_offce = null;
$divi_offce2 = null;
$shopro_dept_id = ($_GET['q']);

if (isset($_SESSION["me_accss_level"])) {
    $user_email = $_SESSION['me_user_email'];
    $uleveldis = $_SESSION["me_accss_level"];
    $province = $_SESSION["me_province"];
    $ministry = $_SESSION["me_ministry"];
    $district_offce = $_SESSION["me_district_offce"];
    $divi_offce = $_SESSION["me_offce"];
    $divi_offce2 = $_SESSION["me_offce2"];
} else {
    
}
?>
<div class="form-group">
    <label for="sabhawa" class="col-md-3 control-label">Zonal Office</label>
    <div class="col-md-9">
        <select type="text" class="form-control Signup_Frm Signup_zonal_s" id="Signup_zonal_s"placeholder="Zonal Office" onclick="clk_divid()">
            <option value='' >--Non--</option>
            <?php
            if (isset($_SESSION["me_accss_level"])) {
                $sql = null;

                $sql = "SELECT `znal_id`, `zonal_name` FROM `tbl_zonal_offce`WHERE `province_code` =  '$province' AND `dept_id` =  '$shopro_dept_id'ORDER BY `zonal_name`";

                if ($db->getresultset($sql)) {
                    $result = $db->getresultset($sql);
                    while ($erow2 = $result->fetch_assoc()) {
                        ?>
                        <option value=<?php echo $erow2["znal_id"]; ?> ><?php echo $erow2["znal_id"]; ?> - <?php echo $erow2["zonal_name"]; ?></option>
                        <?php
                    }
                }
            }
            ?>
        </select>
        <input hidden class="Signup_Frm Signup_zonal" id="Signup_zonal"name="Signup_zonal" value="null"></input>
    </div>
</div>
